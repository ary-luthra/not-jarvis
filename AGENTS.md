...
**not-jarvis** is a personal AI assistant powered by the OpenAI Responses API. It runs as a Slack bot via Socket Mode (no public URL needed), responds to @mentions and DMs, maintains per-thread conversation context, supports per-user long-term memory, and can delegate computer tasks to background Codex sessions.
...
- **Codex sessions = hands.** Anything that takes time (file CRUD, shell commands, browser automation, complex multi-step tasks) is dispatched to background Codex subprocesses via `session_manager.py`. These run in a sandboxed directory and their output can be checked later.
...
- `dispatch_computer_task` — spawns a background Codex session (backed by `session_manager.py`)
